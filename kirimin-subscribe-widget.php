<?php
/*
Plugin Name: Kirimin Subscribe Widget
Plugin URI: http://www.kirim.in
Description: Menambahkan subscribe widget untuk aplikasi <a href="http://www.kirim.in/" target="_blank">Kirimin Email Marketing</a> anda. <br />Menggunakan widget, anda dapat menghasilkan formulir berlangganan berdasarkan email list yang telah anda definisikan. Anda juga memiliki kontrol penuh pada generated form, sehingga anda dapat terus mengubah hingga sesuai dengan kebutuhan anda.
Version: 1.0
Author: Kirimin Developer <admin@kirim.in>
Author URI: http://www.kirim.in
License: MIT http://opensource.org/licenses/MIT
*/

// register the sdk autoloader.
if (!class_exists('MailWizzApi_Autoloader', false)) {
    require_once dirname(__FILE__) . '/kirimin-php-sdk/KiriminApi/Autoloader.php';
...
class MailWizzNewsletterBox extends WP_Widget 
{
    ...
    public function __construct()
    {
        parent::__construct(
            'kmznb', // Base ID
            __('Kirimin Subscribe Widget', 'kmznb'), // Name
            array( 'description' => __( 'Kirimin Subscribe Widget', 'kmznb' ), ) // Args
        );
    }
    ...
    public function form($instance) 
    {
        ...
        $freshLists = array(
            array('list_uid' => null, 'name' => __('Silahkan pilih', 'mwznb'))
        );
        $freshFields = array();
        ...
        <p>
            <input class="widefat mwz-api-url" id="<?php echo $this->get_field_id('api_url'); ?>" name="<?php echo $this->get_field_name('api_url'); ?>" type="hidden" value="<?php echo esc_attr($apiUrl); ?>" />
        </p>
        ...
        <div class="lists-container" style="<?php echo !empty($freshFields) ? 'display:block':'display:none';?>; margin:0; float:left; width:100%">
            <label for="<?php echo $this->get_field_id('list_uid'); ?>"><strong><?php _e('Pilih list:'); ?></strong></label>
        ...
        <div class="fields-container" style="<?php echo !empty($listUid) ? 'display:block':'display:none';?>; margin:0; float:left; width:100%">
            <label for="<?php echo $this->get_field_id('selected_fields'); ?>"><strong><?php _e('Field:'); ?></strong></label>
            ...
            <div style="float: right;">
                Generate form ulang: <input name="<?php echo $this->get_field_name('generate_new_form'); ?>" value="1" type="checkbox" checked="checked"/>
            </div>
            <br class="clear">
        </div>
    ...
    public function update($new_instance, $old_instance) 
    {
        ...
        $instance['api_url']        = 'http://app.kirim.in/api';
...
function mwznb_admin_notice() {
    ...
    if (is_writable($cacheDir = dirname(__FILE__) . '/kirimin-php-sdk/KiriminApi/Cache/data/cache')) {
        return;
    }
    ?>
    <div class="error">
    ...
    return new MailWizzApi_Config(array(
        ...
        'components' => array(
            'cache' => array(
                ...
                'filesPath' => dirname(__FILE__) . '/kirimin-php-sdk/KiriminApi/Cache/data/cache', // make sure it is writable by webserver
            )
        ),
    ));
}
...
function mwznb_generate_fields_table(array $freshFields = array(), $fieldName, array $listSelectedFields = array()) {
    ...
    <table cellpadding="0" cellspacing="0">
        <thead>
            <th width="40" align="left"><?php echo  __('Tampilkan', 'mwznb');?></th>
            <th width="60" align="left"><?php echo  __('Wajib', 'mwznb');?></th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td width="60" align="left"><?php echo str_replace('no', 'tidak', str_replace('yes', 'ya', $field['required']));?></td>
